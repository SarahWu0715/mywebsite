...
![My Photo](/Users/sarahhh/Downloads/pre_programme_assignment/WechatIMG310.jpeg)
...
my_continent_data <- gapminder %>% 
            filter(continent == "Asia")

ggplot(my_continent_data, mapping = aes(x = year , y = lifeExp, colour= country, group = country))+
   geom_point() + 
   geom_smooth(se = FALSE) +
   labs(title = "Life Expectancy Over Time in Asia by Country",
       x = "Time(Year)",
       y = "Life Expectancy") 
```
...
Answer: In general, life expectancy in each continent has been increasing after 1952. However, some African countires have experienced decreases in life expectancy starting 1980, which may be due to the HIV/AIDS epidemic swept Africa. 
...
ggplot(brexit_results, aes(x = born_in_uk, y = leave_share)) +
  ...
  theme_bw() +
  labs(title = "Correlation Between Brexit and Immigration",
       x = "Proportion of Native Residents",
       y = "Percentage of Votes in Favour of Brexit") 
```
...
Answer: There is a moderately positive relationship between the percentage of votes in favour of Brexit and proportion of native residents. Most constituencies tend to have a large proportion of native residents (90%-100%) and relatively large percentage of votes in favour of Brexit (40%-70%). 
...
Answer: The mean incident costs are generally higher than the median, which suggests that the distribution of costs should be skewed to the right. One outlier is the minimum incident cost for dogs, which is zero. 
...
The second graph, in my opinion, best conveys the variability of incident natonal costs for saving different types of animals. From the charts, we can see that horses can be more expensive to rescue than others, with a large spread of possible values from 0 to 3000 and a majority of density below 1500. 
